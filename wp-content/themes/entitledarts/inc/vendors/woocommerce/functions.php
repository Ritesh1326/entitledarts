...
if ( !function_exists('tumbas_woocommerce_enqueue_styles') ) {
    function tumbas_woocommerce_enqueue_styles() {
        ...
        $alert_message = array(
            'success'       => sprintf( '<div class="woocommerce-message">%s <a class="button btn btn-primary btn-inverse wc-forward" href="%s">%s</a></div>', esc_html__( 'Produtos adicionados ao seu carrinho.', 'tumbas' ), wc_get_cart_url(), esc_html__( 'Ver Carrinho', 'tumbas' ) ),
...
add_filter('add_to_cart_fragments', 'tumbas_woocommerce_header_add_to_cart_fragment' );
    ...
    function tumbas_product_tabs($tabs) {
        ...
        if ( tumbas_get_config('show_product_specification_tab', true) && get_post_meta( $post->ID, 'apus_product_specification', true ) ) {
            $tabs['specifications'] = array(
                'title' => esc_html__('ESPEFICAÇÕES', 'tumbas'),
                'priority' => 15,
...
add_filter( 'loop_shop_per_page', 'tumbas_set_loop_shop_per_page', 20 );
