<?php
//convert hex to rgb
if ( !function_exists ('entitledarts_getbowtied_hex2rgb') ) {
	function entitledarts_getbowtied_hex2rgb($hex) {
		$hex = str_replace("#", "", $hex);
...
if ( !function_exists ('entitledarts_custom_styles') ) {
	function entitledarts_custom_styles() {
		global $post;	
		...
		<style>
			...
			/* check main color */ 
			<?php if ( entitledarts_get_config('main_color') != "" ) : ?>
				...
				{
					border-color: <?php echo esc_html( entitledarts_get_config('main_color') ) ?>;
				}
				...
				{
					background: <?php echo esc_html( entitledarts_get_config('main_color') ) ?>;
				}
				/* setting color*/
				...
				{
					color: <?php echo esc_html( entitledarts_get_config('main_color') ) ?>;
				}
				...
				.text-theme
				{
					color: <?php echo esc_html( entitledarts_get_config('main_color') ) ?> !important ;
				}
				...
				{
					background: <?php echo esc_html( entitledarts_get_config('main_color') ) ?> !important;
				}
			<?php endif; ?>

			
			/* Custom CSS */
			<?php if ( entitledarts_get_config('custom_css') != "" ) : ?>
				<?php echo entitledarts_get_config('custom_css') ?>
			<?php endif; ?>
...
?>
<?php add_action( 'wp_head', 'entitledarts_custom_styles', 99 ); ?>